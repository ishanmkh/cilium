...
node when the backend is located on a remote node. This acting as a hairpinned load
balancer can be handled by Cilium at the XDP (eXpress Data Path) layer where BPF is
...
suitable for testing drivers which do not have XDP support. The majority of drivers
supporting 10G or higher rates also support native XDP on a recent kernel. For cloud
based deployments most of these drivers have SR-IOV variants that support XDP as well.
